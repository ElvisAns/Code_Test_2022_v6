...
class BookingRepository extends BaseRepository
{
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            $data['job_type'] = $consumer_type == 'rwsconsumer' ? 'rws' : ($consumer_type == 'ngo' ? 'unpaid' : 'paid');
            ...
            $this->sendNotificationTranslator($job, $data, '*'); // better handle this with event subscribption elsewehere// send Push for New job posting
        } else {
            $response['status'] = 'fail';
    ...
    public function storeJobEmail($data)
    {
        ...
        $job = Job::findOrFail($data['user_email_job_id'] ?? null); //null id will not be found
